...
| **dbr.save**  Actualiza o inserta una fila en una tabla de la base de datos con base en su sys_pk considerando la versión de instancia de registro (sys_recver) y retorna una estructura con la información adicional mínima no proporcionada de los campos de control. **ref registro=dbr.save(db, tabla, registro)**| **db** – Referencia a una conexión de bases de datos  **tabla** – Cadena con el nombre de la tabla  **registro**  - Referencia a un registro que contiene los datos|
